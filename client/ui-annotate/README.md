# @mydsh/ui-annotate (preview — read this first)
...
```bash
dsh plugin --profile web add @mydsh/ui-annotate@preview

# pnpm 9 refuses a root add (ERR_PNPM_ADDING_TO_ROOT) — pass -w:
#   dsh plugin --profile web add -w @mydsh/ui-annotate@preview
...
- Uninstall: `dsh plugin --profile web remove @mydsh/ui-annotate`, then reload the tab.
...
安装：`dsh plugin --profile web add @mydsh/ui-annotate@preview`（pnpm 9 会要求加 `-w`），然后刷新页面。
...
卸载：`dsh plugin --profile web remove @mydsh/ui-annotate`，然后刷新页面（批注还在 localStorage 里）。
